# tests/fixtures/terraform-bad-property-type/main.tf
# Bad-fixture variant for test-terraform-plan.sh: declares a custom_property
# with an unsupported `type = "datetime"`. The landing-page module's variable
# validation block must reject this at plan time so the failure surfaces
# before any HubSpot API call. Without the validation block, HubSpot would
# return a 400 mid-apply, leaving the plan partially applied.
#
# The corresponding test asserts terraform plan fails with the validation
# error message and that no plan file is produced.
...
module "landing_page" {
  ...
  # The offending entry — type = "datetime" is not in the allow-list
  # {string, number, enumeration, bool}. Plan must fail.
  custom_properties = [
    {
      name      = "signup_date"
      label     = "Signup Date"
