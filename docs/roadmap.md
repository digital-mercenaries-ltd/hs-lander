...
- **v1.9.1 — operator ergonomics + tests (plan written, pending).** Five additive fixes in `docs/superpowers/plans/2026-04-28-v1.9.1-operator-ergonomics-and-tests.md`: (1) `PREFLIGHT_VERSION_DRIFT` check (closes the v1.8.1 scaffold-pin recurrence path at preflight time), (2) `scripts/migrate-project.sh` with a versioned `migration-rules.sh` lib (replaces manual `?ref=` bumps + module-input wiring), (3) `scripts/projects-describe.sh` (structured-output equivalent of `Read`-ing the project profile, mirrors `accounts-describe.sh`), (4) scaffold-rooted plan test (catches scaffold-root drift like v1.8.1's stuck `?ref=v1.6.0`), (5) `VERSION.compat` maintenance process + CI guard. Non-breaking; preflight contract grows from 17 → 18 lines.
