...
- **v1.8.1 ship — review-and-deploy defects (plan written, ready to implement).** Surgical patch addressing v1.8.0 defects from three independent sources: codex code review (8 fixes), architectural review (docs drift), and Heard v1.8.0 deploy session (3 Starter-blocking bugs B1/B2/B5). Twelve fixes total, all sub-10-line single-file edits except the docs re-sync. Highlights: scaffold module pin bump from `?ref=v1.6.0`; scaffold root variable wiring for `auto_publish_welcome_email` / `email_preview_text` / `capture_post_submit_action_override` / `email_reply_to`; make `EMAIL_REPLY_TO` reachable end-to-end (was dead code in v1.8.0); regenerate `account-setup/.terraform.lock.hcl`; centralise account/project name validation; `post-apply.sh` honours `HS_LANDER_CONFIG_DIR`; `build.sh` sed-escapes; CI shellcheck recurse into `scripts/lib/`; `docs/framework.md` + `CLAUDE.md` re-sync. Plus B1 (`dropdown` fieldType correction), B2 (`bool` properties emit canonical True/False options), B5 (`survey_form depends_on custom_property` to eliminate the two-pass apply). Plan: `docs/superpowers/plans/2026-04-27-v1.8.1-codex-review-fixes.md`.
- **B3 — welcome-email PATCH against published state (stub plan written, probe needed).** Architecturally substantial: needs `terraform_data` orchestration of unpublish→PATCH→republish via `local-exec`, plus probe of `/draft` endpoint behaviour and idempotency of each step. Stub plan captures problem + design candidates + Heard's interim workaround. Likely target v1.8.2 (if shipped soon) or v1.9.0 (folded with the lib + safety pair). Stub: `docs/superpowers/plans/2026-04-27-welcome-email-published-state-handling.md`.
...
| v1.6.5 | — | Marketing Email v3 POST rejects `state = "AUTOMATED"` on create (must create as `AUTOMATED_DRAFT` then POST `/publish`). Create payload moved to `AUTOMATED_DRAFT` / `isPublished = false`; new `terraform_data.publish_welcome_email` resource fires `POST /publish` via `hs-curl.sh` keyed on the email's ID. Plus `scripts/upload.sh` endpoint `/developer/content` → `/published/content` (the `developer` environment was rejected with HTTP 415 by CMS v3 source-code API). |
